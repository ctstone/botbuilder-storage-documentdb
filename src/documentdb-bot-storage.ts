...
export class DocumentDbBotStorage implements IBotStorage {
    ...
    private readData(keys: {userData?:any, privateConversationData?:any, conversationData?:any}, callback: (err: QueryError, data: IBotStorageData) => void): void {
    ...
    private writeData(docs: Array<{id:string, data:any}>, callback: (err: QueryError) => void): void {
      async.eachLimit(docs, this.maxConcurrency, (doc, next: (err: QueryError) => void) => {
        ...
        doc.data = doc.data || {};
        this.client.upsertDocument(`dbs/${this.databaseName}/colls/${this.collectionName}`, doc, { partitionKey:partitionKey, disableAutomaticIdGeneration:true }, next);
      }, callback);
    }
    ...
    private mapDocumentDbError<T>(callback: (err: Error, resource:any) => void): (err: QueryError, resource?:T, headers?:any) => void {
      return (err: QueryError|Error|{message:string}, resource?:T, headers?:any) => {
        if (err instanceof Error) {
          ...
          callback(new Error((<{message:string}>err).message), null);
        } else if (err && err.hasOwnProperty('code')) {
          callback(new Error(`${(<QueryError>err).code}: ${(<QueryError>err).body}`), null)
        } else if (err) {
